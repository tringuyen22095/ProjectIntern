...
@Repository
public interface TransactionExpenseDao extends JpaRepository<TransactionExpense, Integer> {
	 ...
	 * @return
	 */
	@Query(nativeQuery = true, value = "SELECT * FROM transaction_expense a WHERE a.status = 'active' AND UPPER(a.name)"
			+ " LIKE UPPER(CONCAT('%', :name, '%')) AND (a.owner = :owner OR a.owner is null)"
			+ " ORDER BY a.create_date ASC")
	public List<TransactionExpense> getTransactionExpenseLikeName(@Param("owner") String owner,
	 ...
	 * @return
	 */
	@Query(nativeQuery = true, value = "SELECT * FROM transaction_expense a WHERE a.id = :id")
	public TransactionExpense getTransactionExpenseById(@Param("id") int id);
